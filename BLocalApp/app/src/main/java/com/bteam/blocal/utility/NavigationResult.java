...
public class NavigationResult {
    ...
    public static <T> void setNavigationResult(Fragment f, @Nullable String key, T value){
        NavBackStackEntry backStackEntry = NavHostFragment.findNavController(f).getPreviousBackStackEntry();
    ...
    public static <T> LiveData<T> getNavigationResult(Fragment f, @Nullable String key){
        NavBackStackEntry backStackEntry = NavHostFragment.findNavController(f).getCurrentBackStackEntry();
        if(backStackEntry != null){
            SavedStateHandle handle = backStackEntry.getSavedStateHandle();
     ...
     */
    public static <T> void getDialogNavigationResult(Fragment f, @IdRes Integer id, @Nullable String key, IOnResultCallback<T> onResult){
        NavBackStackEntry backStackEntry = NavHostFragment.findNavController(f).getBackStackEntry(id);
        LifecycleEventObserver observer = new LifecycleEventObserver() {
            @Override
            public void onStateChanged(@NonNull LifecycleOwner source, @NonNull Lifecycle.Event event) {
                if(event == Lifecycle.Event.ON_RESUME && backStackEntry.getSavedStateHandle().contains(key)){
                    SavedStateHandle handle = backStackEntry.getSavedStateHandle();
        ...
        f.getViewLifecycleOwner().getLifecycle().addObserver(new LifecycleEventObserver() {
            @Override
            public void onStateChanged(@NonNull LifecycleOwner source, @NonNull Lifecycle.Event event) {
